...
def evaluate_blueprint(blueprint, step_count) -> int:
    initial_state = State(
    ...
    for i in range(1, step_count + 1):
        ...
        # sort states with key lambda equal to sum of all values in state
        states = sorted(states, key=lambda state: sum(state.robots.values()) + sum(state.resources.values()), reverse=True)[-100000:]
        #print(
...
if DO_PART_ONE:
    ...
    for i, _blueprint in enumerate(blueprints, start=1):
        print(f"Blueprint #{i}...")
        most_geodes = evaluate_blueprint(_blueprint, 24)
        print()
...
for i, _blueprint in enumerate(blueprints, start=1):
    print(f"Blueprint #{i}...")
    most_geodes = evaluate_blueprint(_blueprint, 32)
    print()
    result *= most_geodes
print(f"Part two: {result}")

part2_time = time.time()
print(f"Part two took {part2_time - part1_time:.2f} seconds.")
"""
...
def part_1():
    ...
    with mp.Pool(processes=len(blueprints)) as pool:
        ...
        for bp in blueprints:
            results.append(pool.apply_async(evaluate_blueprint, (bp, 24)))
...
def part_2():
    ...
    with mp.Pool(processes=3) as pool:
        ...
        for bp in blueprints[:3]:
            results.append(pool.apply_async(evaluate_blueprint, (bp, 32)))
